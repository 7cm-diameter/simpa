...
generate_probe_trial(10, 120, 20)
...
async def stimulate(agent: Stimulator, expvars: Experimental) -> None:
    ...
    omissions = blockwise_shuffle(generate_probe_trial(10, 120, 20), 10)
    ...
    try:
        ...
        while agent.working():
            for trial, interval in trials:
                print(f"Trial: {trial}, Omission: {omissions[trial]}")
                ...
                if omissions[trial]:
                    agent.send_to(RECORDER, timestamp(OMISSION))
                else:
