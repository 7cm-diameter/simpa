...
async def reward(agent: Stimulator, expvars: Experimental) -> None:
    ...
    mean_iti = expvars.get("mean-iti", 20.0)
    range_iti = expvars.get("range-iti", 5.0)
    inter_reward_intervals = uniform_intervals(mean_iti, range_iti, trial)

    trials = TrialIterator(list(range(trial)), inter_reward_intervals)
...
async def sound(agent: Agent, expvars: Experimental):
    ...
    trial = expvars.get("trial", 120)
    mean_iti = expvars.get("mean-iti", 20.0) - cs_duration
    range_iti = expvars.get("range-iti", 5.0)
