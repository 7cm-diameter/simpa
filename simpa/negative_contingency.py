...
async def stimulate(agent: Stimulator, expvars: Experimental) -> None:
    ...
    cs_intervals = uniform_intervals(mean_iti, range_iti, trial)
    us_intervals = uniform_intervals(mean_iti, range_iti, trial)

    trials = TrialIterator(list(range(trial)),
                           list(zip(cs_intervals, us_intervals)))

    try:
        agent.send_to(RECORDER, timestamp(START))
        while agent.working():
            for trial, (cs_interval, us_interval) in trials:
                print(f"Trial: {trial}")
                await agent.sleep(cs_interval)
