...
async def stimulate(agent: Stimulator, expvars: Experimental) -> None:
    ...
    presentations = blockwise_shuffle(generate_probe_trial(10, 120, 20), 10)
    ...
    try:
        ...
        while agent.working():
            for trial, interval in trials:
                print(f"Trial: {trial}, Presentation: {presentations[trial]}")
                if presentations[trial]:
                    await agent.sleep(interval - 5.)
                    agent.send_to(RECORDER, timestamp(1200))
