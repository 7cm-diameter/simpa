...
# SAVE_DIR <- "./data/bouts"
# filepaths <- (function() {
#   contingent <- list.files("./data/events", pattern = c("training"), full.names = T)
#   noncontingent <- list.files("./data/events", pattern = c("_non-contingent_"), full.names = T)
#   c(contingent, noncontingent)
# })()
# 
# # data <- read.csv(filepaths[[20]]) %>% extract_fr2lr
...
# # 
# # 
# # stanfit <- rstan::read_stan_csv(fitted$output_files())
...
#     l <- list(data = saved_data, path = saved_path, convergence = convergence, estim = estim)
