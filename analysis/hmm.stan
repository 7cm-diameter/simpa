...
data {
  int<lower=1> T; //num obs
  int<lower=1> K; // num state
  real y[T]; // obs data
}
...
transformed parameters {
...

  {
    ...
    logalpha[1] = log(pi1) + exponential_lpdf(y[1]| lambda);

    for (t in 2:T){
      for(j in 1:K){
        for(i in 1:K){
          accumulator[i] = logalpha[t-1, i] + log(A[i, j]) + exponential_lpdf(y[t] | lambda[j]);
        }
...
generated quantities {
    ...
    for (tforward in 0:(T-2)) {
      ...
      for (j in 1:K) { // j = previous (t-1)
        for (i in 1:K) { // i = next (t)
          ...
          accumulator[i] = logbeta[t, i] + log(A[j, i]) + exponential_lpdf(y[t] | lambda[i]);
          }
  ...
  { // Viterbi algorithm
    ...
    for (j in 1:K)
      delta[1, K] = exponential_lpdf(y[1] | lambda[j]);
    ...
    for (t in 2:T) {
      for (j in 1:K) { // j = current (t)
        ...
        for (i in 1:K) { // i = previous (t-1)
          real logp;
          logp = delta[t-1, i] + log(A[i, j]) + exponential_lpdf(y[t] | lambda[j]);
          if (logp > delta[t, j]) {
            bpointer[t, j] = i;
            delta[t, j] = logp;
          }
